...
- 0fae613: WebhookCard sits on its own surface now: the app background inside its hairline border, so a hovered message row stops at the card's edge instead of showing through it. Field chips and picture placeholders use the raised surface. A click or tap anywhere on the card opens its link, with a pointer, a hover state and the focus ring on the card itself. Links, buttons and pictures inside the card still open on their own. A drag that selected text opens nothing, and only http(s) URLs become links.

## 0.34.3

### Patch Changes

- 8a93091: Dialog and AlertDialog no longer grow past the browser viewport. Their popups keep the same 1.5rem edge clearance they already use horizontally, cap against the dynamic viewport height, and scroll vertically when their content is taller than the available space.

## 0.34.2

### Patch Changes

- 79ebfe3: `Select` doesn't run off the bottom of the window any more when it has a lot of options. The popup stops at the edge of the window and never grows past 24rem. The list scrolls inside it, and the popup keeps its padding, corners and group labels.
...
- ebd13a6: `Select` cuts a long selected label off with an ellipsis now. It used to wrap onto two or more lines, centred in the trigger, and a label with no spaces or hyphens set the width of the whole Select.
...
- 09144e0: An unread colour of its own, and a Badge tone that uses it.
...
- d53a79c: Button and IconButton keep no underline when they are rendered as a link.
  ...
  Only the control rendered _as_ the anchor needs this. One sitting inside a link
...
- 93a95a6: Style the menu parts that were being handed through from Base UI unstyled.
...
- b679291: The component-layer rules ship on their own, as components.css
...
- c470f2f: The owl designer lays itself out from its own width, not the window's
...
- e9fc69e: The owl designer moves into the library.
...
- Updated dependencies [cef19b9]
- Updated dependencies [fb817a9]
  - @gryt/theme@0.9.0
...
- c2d2a46: `ThemeEditor`, the theme controls as a component.
