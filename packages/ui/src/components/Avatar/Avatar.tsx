...
// The intrinsic size of the generated SVG, which CSS then overrides. One number for every
// size: it is vector, and a single value means one memo entry per person.
const OWL_SIZE = 256;
...
> {
  ...
  className?: string;
  /**
   * Draws this person's owl when there is no `src`. Pass `avatarSeed(nickname)`, not the
   * nickname: the two differ for anyone whose name is not lower case.
   */
  seed?: string;
  /**
   * Draws this seed's eggs. Named for the drawing rather than what it stands for, and
   * separate from `seed` so the wrong prop gives the wrong kind of thing.
   */
  eggSeed?: string;
