...
export function rgbToHex([r, g, b]) {
    const part = (c) => Math.round(clamp01(c) * 255)
        .toString(16)
        .padStart(2, "0");
    return `#${part(r)}${part(g)}${part(b)}`;
}
export function hexToOklch(hex) {
    const [r0, g0, b0] = hexToRgb(hex).map(srgbToLinear);
 ...
 */
export function hueScale(solid, solidHover) {
...
export function neutralScale(anchors) {
    ...
    return NEUTRAL_L.map((l, i) => fixed[i] ?? oklchToHex({ l, c: NEUTRAL_C[i], h: hue }));
}
/**
 ...
 */
export function alphaScale(scale, background) {
    ...
    return scale.map((hex) => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            const b = bg[i];
            if (t > b)
                alpha = Math.max(alpha, b < 1 ? (t - b) / (1 - b) : 0);
            else if (t < b)
                alpha = Math.max(alpha, b > 0 ? (b - t) / b : 0);
        }
        if (alpha < 1e-6)
            return "transparent";
        const overlay = rgbToHex(target.map((t, i) => clamp01(bg[i] + (t - bg[i]) / alpha)));
...
export function neutralScaleLight(anchors) {
    ...
    const hue = hexToOklch(anchors.border).h;
    return LIGHT_NEUTRAL_L.map((l, i) => fixed[i] ?? oklchToHex({ l, c: LIGHT_NEUTRAL_C[i], h: hue }));
}
/**
 ...
 */
export function hueScaleLight(solid) {
    const { l, c, h } = hexToOklch(solid);
    const steps = LIGHT_HUE_L.map((sl, i) => oklchToHex({ l: sl, c: LIGHT_HUE_C[i], h }));
    steps.push(solid);
